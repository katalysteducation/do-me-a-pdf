...
def job_add_artifact(request, job_id):
    ...
    if type == 'unknown':
        ...
        raise RuntimeError(
            'type={!r} should not have been accepted'.format(type))
...
def job_media(request, job_id, name):
    ...
    rsp['Content-Disposition'] = 'inline; filename={}'.format(
        smart_str(artifact.name))
...
def add_new_job(request, form):
    ...
    if source == 'collection.zip':
        ...
        raise RuntimeError(
            'collection_source={!r} should not have been accepted'
            .format(source))
